...
public class MemberQueryService implements MemberQueryUseCase {
    ...
    public List<MemberListNotPosResDto> memberlistNotPos() {
        List<Member> members = memberPort.findAll();
    ...
    public List<DriverListResDto> dirverList() {
        List<Driver> drivers = driverPort.findAll();
    ...
    public List<StoreWithPosResDto> storeWithPosList() {
        List<Store> stores = storePort.findAllWithMember();

        List<Integer> storeIds = stores.stream()
                .map(Store::getId)
                .collect(Collectors.toList());

        List<Pos> allPos = posPort.findByStoreIdIn(storeIds);
    ...
    public Page<StoreListResDto> storeList(Pageable pageable, String keyWord, String status) {
        Page<Store> stores = storePort.findAll(pageable, keyWord, status);
    ...
    public StoreWithPosResDto readMyStore(Integer memberId) {
        ...
        if (member.getRole() == MemberRole.POS_MEMBER) {
            Pos pos = posPort.findByMember_Id(memberId);
            store = pos.getStore();
        } else {
            // BRANCH_MANAGER 등 Store owner인 경우
            store = storePort.findByMemberId(memberId);
        }

        List<Pos> posList = posPort.findByStoreIdIn(List.of(store.getId()));
    ...
    public StoreWithPosResDto readOtherStroe(Integer id) {
        ...
        List<Pos> posList = posPort.findByStoreIdIn(List.of(id));
