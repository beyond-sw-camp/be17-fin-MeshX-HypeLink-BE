...
@Tag(name = "직영점 결제 관리", description = "직영점에서 결제를 검증하고 처리하는 API")
@RestController
...
public class PaymentController {
    ...
    @Operation(summary = "결제 검증", description = "결제 정보를 검증하고 재고를 확인합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "검증 성공",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = BaseResponse.class),
                            examples = @ExampleObject(value = DirectStoreSwaggerConstants.PAYMENT_VALIDATION_SUCCESS_RES_EXAMPLE))),
            @ApiResponse(responseCode = "400", description = "검증 실패 (재고 부족 또는 잘못된 데이터)", content = @Content)
    })
    @PostMapping("/validate")
    public ResponseEntity<BaseResponse<String>> validatePayment(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "결제 검증 정보",
                    required = true,
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = PaymentValidationReq.class),
                            examples = @ExampleObject(value = DirectStoreSwaggerConstants.PAYMENT_VALIDATION_REQ_EXAMPLE)
                    )
            )
