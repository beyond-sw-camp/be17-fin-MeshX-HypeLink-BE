...
public interface ItemFeignClient {
    ...
    @CircuitBreaker(name = "ITEM_CIRCUIT", fallbackMethod = "validateItem")
    @GetMapping("/validate")
    String validateItem(@RequestParam String itemCode);

    // 자바 8 에서 추가된 기능 : 인터페이스에 default로 메소드 구현 가능
    default String fallbackValidateItem(Throwable cause) {
        log.error("예외 처리 : {}", cause.toString());
        return "저장에 실패했습니다.";
    }
}
